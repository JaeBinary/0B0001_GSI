...
## Chapter 10_객체 지향 프로그래밍
1. 객체란 이떤 속성과 행동을 가지고 있는 데이터를 말한다.

2. 클래스는 객체는 객체를 만들기 위한 설계도 또는 찍어내는 틀이다.

3. 클래스를 만들기  위한 형식은 다음과 같다.
```python
class 클래스명 :
    # 클래스 생성 코드 구현
```

4. 클래스의 속성은 지금까지 사용했던 변수처럼, 행동은 지금까지 사용했던 함수(Function) 형태로 구현한다. 클래스 안에서 구현된 함수는 메소드(Method)라고 부른다.

5. 클래스를 만든 후에는 객체를 여러 개 만들 수 있고, 객체를 만드는 코드는 다음과 같다.
```python
객체1 = 클래스명()
객체2 = 클래스명()
```

6. 객체의 속성은 **객체명.속성명**으로 접근할 수 있다. 메소드의 호출은 **객체명.메소드명()** 형식을 사용한다.

7. 생성자(Constructor)란 객체를 생성하면 무조건 호출되는 메소드를 의미한다. 생성자는 클래스 안에서 __init__()라는 이름으로 지정되어 있다.

8. 클래스의 특별한 메소드로 객체가 제거될 때 자동으로 호출되는 __del__() 메소드, 객체끼리 덧셈을 할 경우에 실행되는 __add__() 메소드 등이 있다.

9. 상속을 구현하기 위해서는 서브클래스를 정의할 때, 괄호 안에 슈퍼클래스의 이름을 넣어준다.
```python
class 서브클래스명(슈퍼클래스명) :
    # 서브클래스 코드 구현
```
